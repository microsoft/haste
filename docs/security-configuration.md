...
- **Ingestion size/type limits.** Chunked uploads are capped (`HASTE_MAX_UPLOAD_BYTES`, default 5 GiB) and the assembled file's magic bytes must match its declared format before GDAL parses it. Remote imagery fetches are capped (`HASTE_MAX_IMAGERY_DOWNLOAD_BYTES`, default 8 GiB) and refuse cross-host redirects (SSRF guard). Tune the same-named GitHub Environment configuration variables, then run **Update Size Limits**. Both deployment and lightweight updates apply these desired values. Bicep declares the caps explicitly; local `azd` environments must supply matching byte counts before provisioning. The workflow verifies stored settings and samples HTTP worker responses, not queue workers or Batch enforcement. New Batch tasks capture the queues worker's download cap; existing tasks retain their submitted values. See the [operator procedure](configuration.md#ingestion-size-limits) for reset, rollback, resource budgets, and verification.
