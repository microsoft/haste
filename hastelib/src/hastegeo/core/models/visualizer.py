...
class Visualizer(BaseModel):
    ...
    currentPredictionRevision: Optional[str] = Field(default=None)
    predictionVersion: int = Field(default=0)
    predictionVersions: list[dict] = Field(default_factory=list)
    isEdited: bool = Field(default=False)
    threshold: float = Field(default=0)
    unknownThreshold: float = Field(default=0)
    editedCount: int = Field(default=0)
