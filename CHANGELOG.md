...
feel; this is a visual change only, and nothing has moved. Training is
materially more correct than in 2.x — four separate defects meant the "No
Damage" class was never trained the way it was designed to be, and one of them
requires retraining affected models. Deployments running many environments
against scarce GPU quota can now share multi-tenant Batch pools with per-tenant
credential isolation. The Interactive Labeler no longer loses saved labels.

### Upgrade actions

> Read this before upgrading. Details are in the linked entries below.

1. **Retrain any model whose project defines a "No Damage" class.** That class
   previously received an output channel and could be predicted at inference;
   it no longer carries one. Predictions from earlier models are not
   meaningful for "No Damage" areas. See *The "No Damage" constraint loss let
   the model predict "No Damage"*.
2. **Rename the `AZURE_BATCH_REGISTRY_SERVER_URL` application setting to
   `AZURE_BATCH_REGISTRY_SERVER`** on the `api` and `queues` Function Apps
   (value: the bare login server, e.g. `myacr.azurecr.io`), and confirm the
   training/imageryprep pool ids are identical across both apps. See *Batch
   application settings were out of sync with the code that reads them*.
3. **Re-run any image layer that failed with `NodeNotReady`.** The log upload
   that makes those failures diagnosable applies only to newly submitted
   tasks. See *Losing the Batch compute node failed image layers that had
   already succeeded*.
4. **"No Damage" must now be the last entry in a project's class list** for
   the constraint loss to engage; where it isn't, the loss stays off and the
   reason is logged.

### Added
- **Publish a finished damage assessment** — a processed model result can now be *published* from either workflow, to one of two targets: an immutable copy in the app's own storage (downloadable predicted-damage geopackage, building footprints, valid-area mask, and the processed image), or an external **Microsoft Planetary Computer Pro GeoCatalog** as a STAC collection + item, discoverable through the catalog's STAC API and map Explorer. A **Published Datasets** view lists every dataset published in the deployment — not just your own — with publisher, target, and status, searchable and filterable; a dataset's owner or an administrator can edit its metadata and push the change to the live STAC item, retry a failed publish, or unpublish, which removes the item and drops the collection once it becomes empty. A **force-remove** escape hatch handles datasets that can't recover. Publishing is operator-enabled and off by default (`HASTE_PUBLISHING_ENABLED`); the Planetary Computer target additionally needs `HASTE_PC_PROVIDER_ENABLED` plus GeoCatalog URL, ingestion source, and license settings. Targets an administrator hasn't configured are shown in the dialog with the reason they're unavailable. Full design in [`spec/features/data-publishing/`](spec/features/data-publishing). See [docs/usage/data-publishing.md](docs/usage/data-publishing.md) and [docs/configuration.md](docs/configuration.md).
- **Damage-classification layer in the Planetary Computer Explorer** — publishing to a GeoCatalog also rasterizes the damage output into a classification COG and registers the render, mosaic, and tile configuration the Explorer needs, so the collection becomes selectable there and draws damaged buildings in red over undamaged buildings in grey. The layer is HASTE's own derived raster, never the source imagery, so it carries the same license and attribution as the vector outputs. On by default (`HASTE_PUBLISH_EXPLORER_RENDER_ENABLED`); the Explorer cannot display a collection without a render configuration at all, and the vector assets stay downloadable but are not drawn, since the Explorer renders raster layers. Design in [`spec/features/data-publishing/explorer-visualization.md`](spec/features/data-publishing/explorer-visualization.md).
- **Source-imagery provenance on published datasets** — imagery assembled from the Open Data Catalog now records where it came from and carries that provenance onto every dataset published from it, so published results credit the imagery correctly and link back to the exact source scenes. On a Planetary Computer dataset this becomes standard STAC attribution: the imagery vendor as `licensor`, the organization operating the deployment as `producer` and `processor` (`HASTE_PUBLISHING_ORGANIZATION_NAME` / `_URL`), a `derived_from` link per source scene, and an optional free-text or URL citation. Applied only to imagery from **registered open-data programs**, as a licensing safeguard — publishing to the Planetary Computer never redistributes source imagery to the external catalog, only derived outputs. Design in [`spec/features/data-publishing/source-imagery-provenance.md`](spec/features/data-publishing/source-imagery-provenance.md).
...
- **Releases are published from the changelog** — pushing a `v*` tag now publishes a GitHub Release whose notes are that version's `CHANGELOG.md` section, with repo-relative links rewritten to permalinks at the tag. Each release cites the `hastegeo` wheel it ships with — the newest wheel published when the release was cut — linking to the copy on [`haste-binaries`](https://github.com/microsoft/haste/releases/tag/haste-binaries) rather than duplicating it. The eight historical releases (v1.4.1–v2.0.0) have been backfilled from their existing changelog sections, and `haste-binaries` is relabelled as what it is: an append-only asset store for the GDAL and `hastegeo` wheels, not a product release. Versioning and tagging conventions for the three independently versioned artifacts — the product, the `hastegeo` wheel, and the Docker images — are documented in [RELEASING.md](RELEASING.md).

### Changed
- **UI reskinned for an app-like look and feel** — the interface was restyled away from a website-like presentation toward something that reads as an application. This is a look-and-feel change only: the core building blocks, layout, and workflows are unchanged, so nothing has moved and no screen has to be relearned. Existing projects, image layers, and labels are unaffected.
...
- **Ctrl+right-drag labelled instead of clearing in the Interactive Labeler** — right-click clears a single label and Ctrl+drag box-labels, but Ctrl+**right**-drag box-*labelled* too, applying the current class to everything in the box. An accidental gesture silently mislabelled buildings, and clearing in bulk meant one right-click per building. Ctrl+right-drag now box-clears, mirroring right-click, and clears the whole selection in one pass rather than re-tallying per building. ([#118](https://github.com/microsoft/haste/issues/118))

#### Azure Batch and deployment
