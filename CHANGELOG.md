...
- **The "No Damage" constraint loss let the model predict "No Damage"** — the loss penalized only the predicted *Damaged Building* probability at "No Damage" pixels, which under-constrains the problem: every other output channel was left untouched, so the cheapest way to satisfy the objective was to dump probability on the unsupervised "No Damage" channel. Models trained this way emitted "No Damage" as a predicted class at inference, and the nodata channel was equally unconstrained. "No Damage" describes a building rather than anything visible in the imagery, so it is never a valid prediction. The term is now a partial-label objective over the classes that remain possible at such a pixel — everything except *Damaged Building*, the *No Damage* channel itself, and the unlabeled channel — which drives all three to zero while leaving the choice among the real classes free. **Models trained before this change need retraining**; their predictions for "No Damage" areas are not meaningful.
