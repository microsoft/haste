...
- **The Interactive Labeler could load with zero labels, and saving from that state destroyed the saved set** — saved labels are keyed by Overture id, but the labeler's working state (feature vectors, map colouring, the label counts) is keyed by the features sidecar's row index. Those two id spaces only met on a *rendered* vector tile, so the labeler discovered its own labels as the user panned — the panel read `Intact: 0 / Damaged: 0 / Cloudy: 0` on load even when the layer had thousands. Worse, **Save labels** sent only the labels that happened to be hydrated and `PutInteractiveLabels` replaces the stored document wholesale, so saving before every labeled tile had rendered permanently deleted the rest (one report went from 1,311 saved labels to 552). Saving now merges the stored set with the session's, so no label can be dropped, and each saved label records its row index — which lets the next session restore the full set up front, before the map renders anything. Row indices are treated as a hint and re-checked against each tile's `overture_id` as it loads, so a label can never be restored onto the wrong building. Labels saved before this change carry no row index, so they keep restoring progressively; a save preserves them but cannot stamp one on, because nothing on the client maps an Overture id to a row index without a rendered tile. They are upgraded individually, each time one's tile has rendered and the labels are saved. ([#113](https://github.com/microsoft/haste/issues/113), [#101](https://github.com/microsoft/haste/issues/101))
