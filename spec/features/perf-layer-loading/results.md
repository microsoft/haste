...
## Headline metric — logical metadata operations per request

| Fixture | Layers × Models | **Logical metadata operations** | Payload | Formula `3 + L·(2M+2)` |
|---|---|---|---|---|
| small | 5 × 2 | **33** | 5.8 KB | 33 ✓ |
| medium | 20 × 5 | **243** | 49.2 KB | 243 ✓ |
| large | 50 × 5 | **603** | 122.4 KB | 603 ✓ |

Logical metadata operation counts match the derived cost formula exactly, confirming the replay is
...
|---|---|---|
| Logical metadata operations / request | **603** | ≤ ~6 + 2 bounded fan-outs |
...
- Host: Docker Desktop on Apple Silicon; the amd64 API image runs emulated, inflating
  the non-storage (CPU) portion somewhat. The logical metadata operation count (603) is
