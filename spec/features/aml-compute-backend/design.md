...
### Image-layer creation and edit ownership

`PutLayer` accepts `normalizationFactor` as an input when creating an image
layer, including the UI's default string `"0"` and numeric values. Model
validation converts valid inputs to numbers before queueing and saving.
Creation must still reject client-supplied workflow status, progress,
normalization statistics, and output artifact paths.

On edits, the stored `normalizationFactor` remains authoritative along with
the preprocessing handle and other workflow-owned fields; accepting it on
creation must not let an edit rewrite processed imagery state. This contract
applies whether the new layer ID is generated by the API or supplied by a
client and does not depend on the selected compute backend.

### Local output persistence

Local Compose explicitly configures `COMPUTE_OUTPUT_CONTAINER_URL` to the
same Azurite account/container used by metadata and artifacts. The local
adapter passes its configured Blob service URL to the shared URI parser:
path-style account segments stay in the container URL but are excluded
from the container name and blob prefix. Azure Batch's host-style parsing
is unchanged. The same rule applies to folder inputs used by artifact
packaging.

Placeholder output destinations are rejected before local execution.
If a container finishes but output upload fails, the local task fails and
retains its working files; it must not report successful persistence or
allow finalization to discard the only output copy.
