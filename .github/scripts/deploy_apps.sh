...
TRANSFORMER_INFERENCE_DOCKER_IMAGE="hastetransformerinference:${TRANSFORMER_INFERENCE_IMAGE_TAG:-$TRAINING_IMAGE_TAG}"
...
PC_PUBLISHING_LICENSE="${PC_PUBLISHING_LICENSE:-CC-BY-4.0}"
# Attribution: organization operating this deployment (STAC processor provider).
PUBLISHING_ORGANIZATION_NAME="${PUBLISHING_ORGANIZATION_NAME:-}"
...
deploy_function() {
    ...
    if [ "${SET_APPSETTINGS:-true}" = "true" ]; then
        ...
        az functionapp config appsettings set --name "$FUNCTION_NAME" --resource-group "$RESOURCE_GROUP" --settings \
            ...
            "PC_PUBLISHING_LICENSE=${PC_PUBLISHING_LICENSE}" \
            "PUBLISHING_ORGANIZATION_NAME=${PUBLISHING_ORGANIZATION_NAME}" \
            "PUBLISHING_ORGANIZATION_URL=${PUBLISHING_ORGANIZATION_URL}" \
            "PUBLISH_STORAGE_ACCOUNT_URL=${PUBLISH_STORAGE_ACCOUNT_URL}" \
            "PUBLISH_BLOB_CONTAINER=${PUBLISH_BLOB_CONTAINER}" \
