...
export default function Visualizer({ setModalComponent }) {
  ...
  // The scene exposes maps only after BOTH panes finish ready/setup, and
  // hides the old scene while replacement results load. Keep map readiness
  // separate from the vector artifact/rendering status shown below.
  const mapsReady = scene.maps?.length === 2;
  ...
  return (
    <div
      ...
      ref={containerRef}
      data-map-ready={mapsReady ? "true" : "false"}
    >
