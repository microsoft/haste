...
"""Container-contract tests for docker/training/scripts/set_dirs.sh.
...
class TestSetDirsWorkdirResolution(unittest.TestCase):
    ...
    def test_canonical_only_environment(self):
        with tempfile.TemporaryDirectory() as work_dir:
            ...
            result, post_sed = self._run(
                ...
                "path: AZ_BATCH_TASK_WORKING_DIR/masks/foo.tif\n",
            )
            ...
            self.assertEqual(values["TORCH_HOME"], f"{bash_work_dir}/")
            self.assertIn(f"path: {bash_work_dir}/masks/foo.tif", post_sed)
            # Dirs required by the damage-assessment model must exist.
            self.assertTrue(os.path.isdir(os.path.join(work_dir, "masks")))
            self.assertTrue(os.path.isdir(os.path.join(work_dir, "images")))
    ...
    def test_both_set_different_canonical_wins(self):
        ...
        with tempfile.TemporaryDirectory() as canonical_dir, \
            ...
            self.assertEqual(values["TORCH_HOME"], f"{canonical_bash_dir}/")
            self.assertIn(
                f"path: {canonical_bash_dir}/masks/foo.tif", post_sed
            )
            self.assertNotIn(legacy_bash_dir, post_sed)
            # Dirs are created under the canonical dir, not the legacy one.
            self.assertTrue(
                os.path.isdir(os.path.join(canonical_dir, "masks"))
            )
            self.assertTrue(
                os.path.isdir(os.path.join(canonical_dir, "images"))
            )
            self.assertFalse(os.path.isdir(os.path.join(legacy_dir, "masks")))
            self.assertFalse(
                os.path.isdir(os.path.join(legacy_dir, "images"))
            )
    ...
    def test_canonical_and_legacy_environments_are_deterministically_identical(
        ...
        with tempfile.TemporaryDirectory() as canonical_dir, \
            ...
            # Both resolve HASTE_JOB_WORKDIR/AZ_BATCH_TASK_WORKING_DIR to
            # their own (distinct) work dirs, but the *shape* of the
            # resolution -- both vars set, both equal to each other, sed
            # substitution applied identically -- is identical.
            self.assertEqual(
