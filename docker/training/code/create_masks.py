...
def main() -> None:
    ...
    output_warped_label_fn = os.path.join(
        output_dir, f"{name}_labels_warped.geojson"
    )
    output_cropped_image_fn = os.path.join(
        output_dir, "images", f"{name}_cropped.tif"
    )
    output_buffered_mask_fn = os.path.join(
        output_dir, "masks", f"{name}_buffered.tif"
    )
    all_output_fns = [
        output_mask_fn,
    ...
    if any([os.path.exists(fn) for fn in all_output_fns]) and not overwrite:
        print(
            "Output files already exist, use --overwrite to overwrite them. Exiting."
        )
        return
    ...
    if result.returncode != 0:
        raise RuntimeError(
            f"gdal_rasterize failed with exit code {result.returncode}.\n"
            f"Command: {' '.join(command)}\n"
            f"stderr: {result.stderr}\n"
            f"stdout: {result.stdout}"
        )
    ...
    for i in range(1, len(class_names)):
        ...
        if result.returncode != 0:
            raise RuntimeError(
                f"gdal_rasterize failed for class '{class_names[i]}' with exit code {result.returncode}.\n"
                f"Command: {' '.join(command)}\n"
                f"stderr: {result.stderr}\n"
                f"stdout: {result.stdout}"
            )
