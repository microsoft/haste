...
def get_coordinates(input_fn):
    ...
    if input_fn.endswith(".tif"):
        print(
            "Input filename is a GeoTIFF, using the bounds of the file as the AOI"
        )
...
def save_footprints(footprints, output_dir, footprint_source, country_code):
    ...
    save_token = os.path.join(
        output_dir,
        f"{country_code}_{footprint_source}_buildings_footprints.gpkg",
    )
...
def main(args):
    ...
    """
    assert args.output_fn.endswith(
        ".gpkg"
    ), "Output filename must end with .gpkg"
